...
if command == 0,  % then initialize the menus/number
   ...
   h_text_elebuoy=uicontrol('Style','text',...
      'String','Buoy & Wght [kg]','FontSize',fs,...
      'Units','Normalized',...
      'Position',[.05 .51 .3 .08]);
   ...
   h_menu_material=uicontrol('Style','popupmenu',...
      ...
      'String',['Steel|Nylon|Dacron|Polypropylene|Polyethylene|Kevlar|Aluminum|Dyneema|Spectra'],...
      'Units','Normalized',...
      'Position',[.45 .22 .4 .08]);
   ...
   if ~strcmp(name,' ') & ~isempty(buoy) & length(dim)== 3 & ~isempty(cd), % OK to add
      if (buoy(1) > -100000 & buoy(1) < 100000 ) & ... % check range of enteries
         ...
         if length(name)>16, disp('Warning: Element Name has too many characters! Limited to 16.'); end
            ...
            tbuoy='                 '; % New buoy now has two numbers: buoyancy and weight (for floats)
            if abs(buoy(1)) < 999, 
