...
for izloop=1:2, % loop through at least twice with this mooring.
    ...
    gamma=1; % The percentage of surface buoyancy used to lift mooring.
    gamma_min=Wt(1)/B(1); % The amount of buoyancy used just to float the surface float.
    ...
    else % this is possibly a surface float mooring
        ss=0;
    ...
    while breaknow==0,  % loop through a few times to converge solution.
        ...
        if isave >= 20, % if it's having problems converging, start a running average when it's close
            ...
            if iavg==1,
                ...
                %if iavg>15, gamma = gammavg/iavg; end
                Z1std=std(Z1);
            end
        end
        ...
        if gamma < 1, % for surface float, just use submerged drag and buoysncy.
            if gamma<gamma_min, gamma_float=gamma_min + gamma; end % This is the total amount of the float submerged 
            Ti(2)=sqrt((gamma_float*Qx(1))^2 + (gamma_float*Qy(1))^2 + (gamma*b)^2);
        end
        %
        %psiNew(2)=real(acos(b/Ti(2)));  % tilt under top element(float)
        T_h=sqrt((gamma_float*Qx(1))^2 + (gamma_float*Qy(1))^2);
        psiNew(2)=real(asin(T_h/Ti(2)));
...
if ~isempty(ZCO), % then we've got clamp-on devices, and we need to find the positions of them
    for jco=1:length(ZCO),  % Iobj now from top to bottom, shifted slightly from center of segment
...
if iconv & ss==0, % adjust heights slightly, bring down into water
    ...
    if abs(zcorr)>0.01, % need to be a little closer then 10 cm!
        ...
        for jco=1:length(ZCO),
            Zfco(jco)=Zfco(jco) + abs(Z(Iobj(jco))/Z10)*zcorr;
        end
    end
end
I=(2:N-1);
