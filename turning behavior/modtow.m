...
global Ht Bt Wt Cdt MEt moorelet Usp Vsp afh
...
if nargin == 0 | command <= 0, % intialize everything
   if command == -1,
      ...
      Ht=[];Bt=[];Wt=[];Cdt=[];MEt=[];moorelet=[];X=[];Y=[];Z=[];Zoo=[];
      H=[];B=[];Cd=[];ME=[];moorele=[];
      Usp=0;Vsp=0;afh=0;
   elseif command == 0,
      if ~isempty(Bt),
...
elseif command==7, % execute the update
   ...
   % add or insert an element
    ...
    Bt(elenum)=BW(1);Wt(elenum)=BW(2); % for floats we need weight (Wt)
