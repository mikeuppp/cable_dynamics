...
%    given a velocity profile U(z) at depths z [m] (+up), with U(0) at surface
%    and a "steady" ship velocity give by Usp & Vsp
...
%          H(1,N) is the height of the towed body/shpere [m]
...
%
iprt=0; % set to 1 for a plot and display of convergence used during de-bugging
%
ss=get(0,'ScreenSize');
...
if isempty(U), % if environmental variable haven't been set
   U=[0 0 0]';V=U;W=U;z=fix(sum(Ht(1,:))*[0 0.2 1.5]');rho=[1024 1025 1026]';
end
if isempty(iss),
...
BwCO=BCO*9.81; % weight of any clamped on devices
...
%
% Note: at the moment, the towed system elements (eg Ht) are listed bottom to top
% but the interpolation to elements "i" (eg Hi) will be top to bottom.
...
Cdi(1)=Cdt(N);
MEi(1)=MEt(N);
z0=Ht(1,N)/2; % initial depth of bottom element (towed body)
% Note all z's are now depths below surface
for i=N-1:-1:1, % setup initial depths of interpolated elements
   j=j+1;
...
% Note now the interpolated towed system
Elindx=j+1-Elindx;
%
...
% Now re-Flip tow cable right side up, interpolated indecies start at bottom.
...
%
% First Pass
% Now we solve for first order wire angles, starting at the bottom of the tow, 
%  where there is no tension from below, Ti=0.
...
for i=2:N-1, % N-1
   ...
   if Ti(ip1) ~= 0, % zero is unlikely, but...
...
%
X(N)=0;Y(N)=0;Z(N)=0;  % reference things to bottom of top device.
dx0=0;dy0=0;dz0=0;
...
while breaknow==0,  % loop through a few times to converge solution.
   ...
   Qy=Qx;Qz=Qx;
   psi1=pi-psi; % we've inverted the towed body to be like a mooring (?)
   for j=1:N, % from bottom up
	  ...
	  i=find(zi>(Z(j)-0.5)&zi<(Z(j)+0.5)); % find the depth of this device
	  if isempty(i), 
      ...
      psi2=psi1(j)-pi/2;    % inverted mooring
      %
      % Now recalculate all the drag forces
      ...
      if ~isempty(ico), % drag for clamp-on devices
	   ...
	   if Hi(3,j)==0, % then this is a tilted cylinder device, now calcualte lift forces
         ...
         Qz(j)=Qz(j) + 0.5*rhoi(i)*CdW*A*sin(psi1(j))*abs(Wi(i))*Wi(i);
       end
      %[2 Qx(j) Qy(j) Qz(j)]
      Qxco=0;Qyco=0;Qzco=0;
      if ~isempty(ico),  % drag for clamp-on devices
   ...
   % [505 180-psi(1)*(180/pi)]
   psi(1)=psi(2); % BUG Nov 2016: was psi(1)=0; This matters for bottom of towed cylinder bodies!!
                  % For sphere objects, this changes nothing, for cylinders it matters.
   % Now Solve tensions and angles from bottom to top
   for i=2:N-1,  % N-1
      ico=[];if ~isempty(Iobj),ico=find(Iobj==i);end % If ~isempty(ico), then there is a clamp on device here
      ip1=i+1;
      ...
      xx=perofB*Qx(i)+Ti(i)*cos(theta(i))*sin(psi(i)); % force in x-direction on next element
	  yy=perofB*Qy(i)+Ti(i)*sin(theta(i))*sin(psi(i)); % force in y-direction on next element
   ...
   %  
   %
   % Now integrate from the top down get the second order [x,y,z] relative
   % to the top [0,0,0]
